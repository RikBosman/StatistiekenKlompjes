...
export default function AdsPdfImport() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      const res = await fetch('/api/ads/import-pdf', { method: 'POST', body: form })
  ...
  async function save() {
    ...
    setSaveState('saving')
    const res = await fetch('/api/ads/invoices', {
      method: 'POST',
    ...
    <div className="space-y-4">
      ...
      >
        ...
        {parsing ? (
          ...
          <>
            <p className="text-slate-500 text-sm">Sleep een Google Ads factuur PDF hierheen of klik om te kiezen</p>
            <p className="text-slate-400 text-xs mt-1">Maandfactuur van Google Ads — totale kosten excl. BTW worden automatisch uitgelezen</p>
          </>
        )}
      </div>
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 space-y-4">
          ...
          {(!parsed.year || !parsed.month || !parsed.amountExclBtw) && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-700">
              Niet alles kon automatisch worden uitgelezen — vul de ontbrekende velden zelf in en controleer het bedrag.
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1">Totale kosten excl. BTW (€)</label>
              <input
                type="number"
                step="0.01"
                value={amount}
